...
const RELEASE = 'GREAT_NECK_NAVIGATOR__GITHUB_DEPLOY__V0.15.0_GN-NAV15.0__SCCC0.9_UEC1.7_REVENUE_ENGINE0.2_PF0.12__UPLOAD_THIS_ZIP.zip';
const EXPECTED_SHA256 = 'fcd54fa1dd52da3079b3aefa63c41bae0e1ef392e1a4a645286f1f1debfe9719';
const EXPECTED_ROOT = 'great-neck-navigator-v0.15.0/';
const EXPECTED_PRODUCT = 'GREAT NECK NAVIGATOR';
const EXPECTED_VERSION = '0.15.0';
const EXPECTED_BUILDER = 'GN-NAV15.0';
const EXPECTED_APP_TREE = '7734fc42164a5de905ebc5d322fa16b14e614faddb6c12c787a7de1bd8db5fc0';
...
if(ident.coordination?.current!=='SMARTER CORE COORDINATION CENTER (SCCC)'||ident.coordination?.release!=='0.9.0'||ident.coordination?.standard!=='UEC-1.7'||ident.coordination?.throughSequence!==21) fail('SCCC identity mismatch');
...
if(product.product!=='GREAT NECK NAVIGATOR'||product.localProductVersion!=='0.15.0'||product.localBuilderVersion!=='GN-NAV15.0') fail('embedded product identity mismatch');
...
const current=JSON.parse(fs.readFileSync(path.join(releaseRoot,'app','governance','CURRENT_RELEASE_TRUTH__v0.15.0.json'),'utf8'));
if(current.version!=='0.15.0'||current.builder!=='GN-NAV15.0'||current.public?.directoryDisplayProfiles!==2043) fail('embedded current-release truth mismatch');
if(current.production?.v015Deployed!==false||current.production?.deploymentAuthorized!==false||current.production?.charging!==false) fail('source must not pre-claim deployment/commerce');
